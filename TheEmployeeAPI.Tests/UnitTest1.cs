...
public class BasicTests(WebApplicationFactory<Program> factory) : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task UpdateEmployee_ReturnsNotFoundForNonExistingEmployee()
    {
        var client = factory.CreateClient();
        var response = await client.PutAsJsonAsync("/employees/99999", new Employee { FirstName = "John", LastName = "Doe", SocialSecurityNumber = "123-45-6789" });
